defmodule AiChefWeb.UserAuthTest do
  use AiChefWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias AiChef.Accounts
  alias AiChef.Accounts.Scope
  alias AiChefWeb.UserAuth

  import AiChef.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, AiChefWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      AiChefWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        AiChefWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
